...
void node::print_routing_table(FILE* file) {
    for(size_t i =0 ; i < this->routing_table.size(); i++) {
        int _num = fprintf(file, "%zu %d %d\n", i, this->routing_table[i].cost, this->routing_table[i].next);
    }
}
